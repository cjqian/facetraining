...
def getRepImg(align, net, imgPath, multiple=False):
    start = time.time()
    bgrImg = cv2.imread(imgPath)
    if bgrImg is None:
        raise Exception("Unable to load image: {}".format(imgPath))
...
def train(args):
    ...
    with open(fName, 'w') as f:
        pickle.dump((le, clf), f, protocol=2)


def loadClassifier(classifierFile):
    with open(classifierFile, 'r') as f:
        (le, clf) = pickle.load(f) 
        align = openface.AlignDlib("openface/models/dlib/shape_predictor_68_face_landmarks.dat")
        net = openface.TorchNeuralNet("openface/models/openface/nn4.small2.v1.t7", imgDim=96,
                                  cuda=False)
        return (align, net, le, clf) 
          
def inferImg(align, net, le, clf, img):
    expected_array = img.split("/")
    reps = getRepImg(align, net, img, False)
    if reps == []:
        return { 
            expected_array[len(expected_array) - 1] : {'found_face': False }
            }

    if len(reps) > 1:
        print("List of faces in image from left to right")

    expected_person = expected_array[len(expected_array) - 2]
    for r in reps:
        rep = r[1].reshape(1, -1)
        bbx = r[0]
        start = time.time()
        ...
        result = [confidence, person, img]
        return {
            expected_array[len(expected_array) - 1] :
            {
                'found_face': True, 
